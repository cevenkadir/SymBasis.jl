# Compare SymBasis vs XDiag.jl: spinless-fermion chain, symmetry-resolved basis construction
# and representative-state lookup.
#
# Run with: julia --project=benchmark benchmark/fermion_basis.jl
...
function symbasis_group(N::Integer, config::Symbol, n_particles::Integer)
    ...
    sg_U1 = sym(TotalSpinlessFermionicNumber(n_particles, N), dofo)
...
function xdiag_block(N::Integer, config::Symbol, n_particles::Integer)
    config == :u1 && return Fermion(N, n_particles)
    ...
    return Fermion(N, n_particles, irrep)
end
...
function run()
    ...
    for N in Ns, config in CONFIGS
        ...
        println("fermion N=$N $(CONFIG_LABELS[config]): dim=$dim_sym  SymBasis=$(sym_mean)±$(sym_std)s  XDiag=$(xdiag_mean)±$(xdiag_std)s")
    ...
    write_csv(
        joinpath(results_dir(), "fermion_construction.csv"),
        [
            "N", "config", "dim",
    ...
    println("fermion representative lookup (N=$N, U1+T+P, $nsamples samples): SymBasis=$(mean_per_call)±$(std_per_call)s/call, XDiag=N/A (no decoupled API)")
    write_csv(
        joinpath(results_dir(), "fermion_representative.csv"),
        [
            "N", "config", "nsamples",
