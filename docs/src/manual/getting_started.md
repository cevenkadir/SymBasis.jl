...
You can also define custom DoF-objects for other kinds of systems — for instance, a system containing flag emojis of countries — by specifying the appropriate parameters via the [`DoFObject`](@ref SymBasis.DoFObjects.DoFObject) constructor from the [`SymBasis.DoFObjects`](@ref dofobjects-api) submodule.

### Symmetry groups
Symmetry groups are the struct SymBasis.jl uses to generate bases with symmetries. You construct them with the [`sym`](@ref SymBasis.SymGroups.sym) function from the [`SymBasis.SymGroups`](@ref symgroups-api) submodule, which covers predefined symmetries such as total magnetization, translation, and reflection. You can also combine multiple symmetries to generate bases that conserve several of them simultaneously.
