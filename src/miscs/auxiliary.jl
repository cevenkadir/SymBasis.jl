...
- `Vector`: A vector of named tuples, each with fields `N0, …, N(n_ldof-1), N` (matching the
    shape expected by [`SymBasis.SymGroups.check_Ns`](@ref)/[`SymBasis.SymGroups.apply_Ns`](@ref)).
"""
...
two agree. It is a required argument because the permutation vector's own element type is an
...
# Arguments
- `perm::AbstractVector{<:Integer}`: The permutation to be wrapped.
...
# Returns
- `Union{BitPermutations.BitPermutation{unsigned(T)}, AbstractVector{<:Integer}}`: The
